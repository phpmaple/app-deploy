...
const add = async file => {
	...
	if (iconFile) {
		try {
			await fixPNG(
				path.join(tmpDir, iconFile.path),
...
const find = (identifierHash, publicURL) => {
	const row = itemInfo(
		appList.find(
			row => hash({ identifier: row.identifier }) === identifierHash
		),
		publicURL
	);
	if (!row) {
